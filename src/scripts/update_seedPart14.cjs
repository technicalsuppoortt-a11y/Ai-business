...
const file = path.join(__dirname, '../services/seedPart14_emailSequences.js');
...
content = content.replace(
  /const getBody = \(emailIndex, goal, audience, tone\) => {[\s\S]*?};\n\n  const subjectList = subjects/g,
...
content = content.replace(
  ...
  `const subjectListObj = subjects[goal.id]?.[tone.id] || subjects.welcome.professional;
  const subjectList = [
    { ar: subjectListObj.ar[0], en: subjectListObj.en[0] },
    { ar: subjectListObj.ar[1], en: subjectListObj.en[1] },
    { ar: subjectListObj.ar[2], en: subjectListObj.en[2] }
  ];`
);

fs.writeFileSync(file, content, 'utf8');
console.log('Modified seedPart14_emailSequences.js');
