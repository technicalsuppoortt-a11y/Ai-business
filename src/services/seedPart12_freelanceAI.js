...
const COL = 'tc_freelance_ai';

const goals = [
  { id: 'acquire', name_en: 'Acquire New Clients', name_ar: 'جلب عملاء جدد' },
  { id: 'close', name_en: 'Close a Deal', name_ar: 'إغلاق صفقة متأخرة' },
  { id: 'objection', name_en: 'Handle Objections', name_ar: 'الرد على اعتراضات' },
  { id: 'retain', name_en: 'Upsell & Retain', name_ar: 'الاحتفاظ بعميل وزيادة المبيعات' }
];

const channels = [
  { id: 'email', name_en: 'Cold Email', name_ar: 'إيميل بارد (Cold Email)' },
  { id: 'social', name_en: 'LinkedIn / Social DM', name_ar: 'رسائل سوشيال/لينكد إن' },
  { id: 'platform', name_en: 'Upwork / Fiverr', name_ar: 'منصات العمل الحر' },
  { id: 'call', name_en: 'Discovery Call', name_ar: 'مكالمة استكشافية (Zoom)' }
];

const clients = [
  { id: 'local', name_en: 'Local Business', name_ar: 'نشاط تجاري محلي' },
  { id: 'ecom', name_en: 'E-commerce', name_ar: 'متجر إلكتروني' },
  { id: 'tech', name_en: 'Tech / SaaS Startup', name_ar: 'شركة تقنية / ناشئة' },
  { id: 'creator', name_en: 'Creator / Influencer', name_ar: 'صانع محتوى / مؤثر' }
];

const tones = [
  { id: 'formal', name_en: 'Professional & Formal', name_ar: 'رسمية واحترافية' },
  { id: 'friendly', name_en: 'Friendly & Consultative', name_ar: 'ودية واستشارية' },
  { id: 'direct', name_en: 'Direct & Aggressive', name_ar: 'مباشرة وبيعية' },
  { id: 'story', name_en: 'Storytelling', name_ar: 'سرد قصصي (Storytelling)' }
];
...
const generateSubjectLine = (goal, channel, client, tone) => {
  if (channel.id === 'call' || channel.id === 'platform') return null; // No subject lines for calls/bids typically, but can be adapted.

  if (goal.id === 'acquire') {
    if (tone.id === 'formal') return `[Proposal] Strategy to Scale ${client.name_ar} Operations`;
    if (tone.id === 'direct') return `Quick question about your ${client.name_ar} growth`;
    if (tone.id === 'story') return `How a similar ${client.name_ar} doubled their revenue`;
    return `Ideas for your ${client.name_ar} - Let's connect`;
  }
  if (goal.id === 'close') {
    return `Next steps for your ${client.name_ar} project`;
  }
  return `Follow up regarding your ${client.name_ar}`;
};
...
export const seedFreelanceAIMatrix = async () => {
  console.log('🌱 Generating Massive Freelance AI Matrix (256 Scenarios)...');
  let count = 0;

  // Save the structure definitions for the UI dropdowns
  const structureData = { id: 'structure_def', goals, channels, clients, tones };
  await setDoc(doc(db, COL, 'structure_def'), structureData);
    ...
    for (const channel of channels) {
      for (const client of clients) {
        for (const tone of tones) {
          const docId = `${goal.id}_${channel.id}_${client.id}_${tone.id}`;
          const content = generateFreelanceMatrix(goal, channel, client, tone);
  ...
  console.log(`✅ Successfully seeded ${count} Freelance Scenarios to '${COL}'`);
  return count;
};
